...
class cascade(nn.Module):
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        self.SA_modules_3 = nn.ModuleList()
        ...
        if cfg.RCNN.USE_RPN_FEATURES:
            ...
            self.xyz_up_layer = pt_utils.SharedMLP([self.rcnn_input_channel] + cfg.RCNN.XYZ_UP_LAYER,
                                                   bn=cfg.RCNN.USE_BN)
            self.xyz_up_layer_3 = pt_utils.SharedMLP([self.rcnn_input_channel] + cfg.RCNN.XYZ_UP_LAYER,
                                                   bn=cfg.RCNN.USE_BN)
            c_out = cfg.RCNN.XYZ_UP_LAYER[-1]
            self.merge_down_layer = pt_utils.SharedMLP([c_out * 2, c_out], bn=cfg.RCNN.USE_BN)
            self.merge_down_layer_3 = pt_utils.SharedMLP([c_out * 2, c_out], bn=cfg.RCNN.USE_BN)
    ...
    def init_weights(self, weight_init='xavier'):
        ...
        nn.init.normal_(self.reg_layer_2nd[-1].conv.weight, mean=0, std=0.001)
        nn.init.normal_(self.reg_layer_3rd[-1].conv.weight, mean=0, std=0.001)
    ...
    def forward(self, input_data):
        ...
        for i in range(len(self.SA_modules_3)):
            li_xyz_3, li_features_3 = self.SA_modules_3[i](l_xyz_3[i], l_features_3[i])
        ...
        if self.training:
            ...
            for i in range(batch_size_2):
                iou_label.append(
                    iou3d_utils.boxes_iou3d_gpu(pred_boxes3d_3rd.view(-1, 7)[i].view(1, 7), gt[i].view(1, 7)))
        ...
        ret_dict = {'rcnn_cls_3rd': rcnn_cls_3rd,'rcnn_reg_3rd': rcnn_reg_3rd,'pred_boxes3d_1st': pred_boxes3d_1st, 'pred_boxes3d_2nd': pred_boxes3d_2nd,
                    'pred_boxes3d_3rd': pred_boxes3d_3rd,
                    'pre_iou3': pre_iou3, 'pre_iou2': pre_iou2}
        ret_dict.update(sec)
        ret_dict.update(two)
