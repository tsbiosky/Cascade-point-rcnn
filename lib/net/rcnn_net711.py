...
class RCNNNet(nn.Module):
    ...
    def forward(self, input_data):
        ...
        pred_boxes3d_2nd=decode_bbox_target(pred_boxes3d_1st.view(-1, 7), rcnn_reg_2nd.view(-1, rcnn_reg_2nd.shape[-1]),
                           anchor_size=anchor_size,
                           loc_scope=cfg.RCNN.LOC_SCOPE,
                           loc_bin_size=cfg.RCNN.LOC_BIN_SIZE,
                           num_head_bin=cfg.RCNN.NUM_HEAD_BIN,
        ...
        pts_input_3 = torch.cat((target_dict_3nd['sampled_pts'], target_dict_3nd['pts_feature']), dim=2)
        target_dict_3nd['pts_input'] = pts_input_3
        xyz_3, features_3 = self._break_up_pc(pts_input_3)

        if cfg.RCNN.USE_RPN_FEATURES:
            xyz_input_3 = pts_input[..., 0:self.rcnn_input_channel].transpose(1, 2).unsqueeze(dim=3)
        ...
        pred_boxes3d_3rd = decode_bbox_target(pred_boxes3d_2nd.view(-1, 7), rcnn_reg_3rd.view(-1, rcnn_reg_3rd.shape[-1]),
                                              anchor_size=anchor_size,
                                              loc_scope=cfg.RCNN.LOC_SCOPE,
                                              loc_bin_size=cfg.RCNN.LOC_BIN_SIZE,
                                              num_head_bin=cfg.RCNN.NUM_HEAD_BIN,
