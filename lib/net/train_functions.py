...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        ...
        if cfg.RCNN.ENABLED:
            ...
            disp_dict['3rd_loss=']=tb_dict['loss_three']
            #disp_dict['1st_loss='] = tb_dict['loss_one']
            disp_dict['2nd_loss='] = tb_dict['loss_two']
            loss += rcnn_loss
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        ...
        #rcnn_cls_2nd, rcnn_reg_2nd = ret_dict['rcnn_cls_2nd'], ret_dict['rcnn_reg_2nd']
        #rcnn_cls_3rd, rcnn_reg_3rd = ret_dict['rcnn_cls_3rd'], ret_dict['rcnn_reg_3rd']
        ...
        if cfg.RCNN.LOSS_CLS == 'SigmoidFocalLoss':
            ...
            rcnn_cls_flat_3rd = rcnn_cls_3rd.view(-1)
            ...
            batch_loss_cls_3rd = F.binary_cross_entropy(torch.sigmoid(rcnn_cls_flat_3rd), cls_label, reduction='none')
            #batch_loss_cls=batch_loss_cls/3
            ...
            rcnn_loss_cls_3rd = (batch_loss_cls_3rd * cls_valid_mask).sum() / torch.clamp(cls_valid_mask.sum(), min=1.0)
            '''
        ...
        if fg_sum != 0:
            ...
            loss_loc, loss_angle, loss_size, reg_loss_dict = \
                loss_utils.get_reg_loss(rcnn_reg.view(batch_size, -1)[fg_mask],
                                        ...
                                        get_ry_fine=True)
            loss_loc_2nd, loss_angle_2nd, loss_size_2nd, reg_loss_dict = \
                loss_utils.get_reg_loss(rcnn_reg_2nd.view(batch_size, -1)[fg_mask],
                                        gt_boxes3d_ct.view(batch_size, 7)[fg_mask],
                                        loc_scope=cfg.RCNN.LOC_SCOPE,
                                        loc_bin_size=cfg.RCNN.LOC_BIN_SIZE,
                                        num_head_bin=cfg.RCNN.NUM_HEAD_BIN,
                                        ...
                                        loc_y_scope=cfg.RCNN.LOC_Y_SCOPE, loc_y_bin_size=cfg.RCNN.LOC_Y_BIN_SIZE,
                                        get_ry_fine=True)
            loss_loc_3rd, loss_angle_3rd, loss_size_3rd, reg_loss_dict = \
                loss_utils.get_reg_loss(rcnn_reg_3rd.view(batch_size, -1)[fg_mask],
                                        gt_boxes3d_ct.view(batch_size, 7)[fg_mask],
                                        loc_scope=cfg.RCNN.LOC_SCOPE,
                                        loc_bin_size=cfg.RCNN.LOC_BIN_SIZE,
                                        num_head_bin=cfg.RCNN.NUM_HEAD_BIN,
            ...
            rcnn_loss_reg_2nd = loss_loc_2nd + loss_angle_2nd + 3*loss_size_2nd
            rcnn_loss_reg_3rd = loss_loc_3rd + loss_angle_3rd + 3 * loss_size_3rd
            tb_dict.update(reg_loss_dict)
            '''
        else:
            loss_loc = loss_angle = rcnn_loss_reg_2nd=rcnn_loss_reg_3rd=loss_size = rcnn_loss_reg = rcnn_loss_cls * 0
        if cfg.TRAIN.IOU_LAYER == 'split':
            rcnn_loss_iou = ret_dict['rcnn_iou_loss']
            rcnn_loss=rcnn_loss_iou
            tb_dict['rcnn_loss_cls'] = rcnn_loss_iou.item()
            tb_dict['rcnn_loss_reg'] = rcnn_loss_iou.item()
            tb_dict['rcnn_loss'] = rcnn_loss.item()
            # tb_dict['loss_one']=one.item()
            tb_dict['loss_two'] = rcnn_loss_iou.item()
            tb_dict['loss_three'] = rcnn_loss_iou.item()
            tb_dict['rcnn_reg_fg'] = rcnn_loss_iou.item()
        else:
            #rcnn_loss_cls=ret_dict['rcnn_loss_cls']
            rcnn_loss_cls_2nd=ret_dict['rcnn_loss_cls_2nd']
            rcnn_loss_cls_3rd=ret_dict['rcnn_loss_cls_3rd']
            #rcnn_loss_reg=ret_dict['rcnn_loss_reg']
            rcnn_loss_reg_2nd=ret_dict['rcnn_loss_reg_2nd']
            rcnn_loss_reg_3rd=ret_dict['rcnn_loss_reg_3rd']
            rcnn_loss_iou=ret_dict['rcnn_iou_loss']
            #rcnn_loss = rcnn_loss_cls+rcnn_loss_cls_2nd + rcnn_loss_cls_3rd+rcnn_loss_reg+rcnn_loss_reg_2nd+rcnn_loss_reg_3rd+rcnn_loss_iou
            rcnn_loss = rcnn_loss_cls_2nd + rcnn_loss_cls_3rd+rcnn_loss_reg_2nd+rcnn_loss_reg_3rd+rcnn_loss_iou


            three=rcnn_loss_cls_3rd+rcnn_loss_reg_3rd
            #one=rcnn_loss_cls+rcnn_loss_reg
            two=rcnn_loss_cls_2nd+rcnn_loss_reg_2nd
            #one=rcnn_loss
            #three=rcnn_loss
            #rcnn_loss=rcnn_loss_cls+rcnn_loss_reg
            #print(rcnn_loss.item())
            tb_dict['rcnn_loss_cls'] = rcnn_loss_cls_3rd.item()
            tb_dict['rcnn_loss_reg'] = rcnn_loss_reg_3rd.item()
            tb_dict['rcnn_loss'] = rcnn_loss.item()
            #tb_dict['loss_one']=one.item()
            tb_dict['loss_two'] = two.item()
            tb_dict['loss_three'] = three.item()
