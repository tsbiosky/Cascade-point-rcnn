...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        if cfg.RPN.ENABLED:
            ...
            if not cfg.RPN.FIXED:
                ...
                rpn_heatmap_lt_label,rpn_heatmap_rb_label=data['heatmap_lt_label'],data['heatmap_rb_label']
                rpn_rb_label,rpn_lt_label=data['rpn_rb_label'],data['rpn_lt_label']
                rpn_heatmap_lt_label=torch.from_numpy(rpn_heatmap_lt_label).cuda(non_blocking=True).float()
                rpn_heatmap_rb_label = torch.from_numpy(rpn_heatmap_rb_label).cuda(non_blocking=True).float()
                rpn_rb_label=torch.from_numpy(rpn_rb_label).cuda(non_blocking=True).float()
                rpn_lt_label = torch.from_numpy(rpn_lt_label).cuda(non_blocking=True).float()
                id2box=data['id2box']
            ...
            #img=torch.from_numpy(data['img']).cuda(non_blocking=True).float()
            #pts_img=torch.from_numpy(data['pts_img']).cuda(non_blocking=True).long()
            #pts_img=data['pts_img']
            ...
            img = torch.from_numpy(data['img']).cuda(non_blocking=True).float()
            #pts_img = torch.from_numpy(data['pts_img']).cuda(non_blocking=True).int()
        ...
        if cfg.RPN.ENABLED and not cfg.RPN.FIXED:
            ...
            offset_lt=ret_dict['offset_lt']
            offset_rb=ret_dict['offset_rb']
            ev=ret_dict['ev']
            #id_lt=ret_dict['id_lt']
            #id_rb=ret_dict['id_rb']
            ori_heatmap_lt=ret_dict['ori_heatmap_lt']
            ori_heatmap_rb=ret_dict['ori_heatmap_rb']
            #####select pts
            select_pts_lt=rpn_heatmap_lt_label.eq(1)  # (B,16384) uint8
            select_pts_rb=rpn_heatmap_rb_label.eq(1)
            offset_lt=offset_lt[select_pts_lt]
            offset_rb=offset_rb[select_pts_rb]
            rpn_lt_label=rpn_lt_label[select_pts_lt]
            rpn_rb_label=rpn_rb_label[select_pts_rb]
            angle_label=rpn_reg_label[:,:,6]
            pri_angle=ret_dict['angle']
            #print(offset_lt.shape,rpn_lt_label.shape)
            #rpn_loss = get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict,input_data['pts_input'])
            rpn_loss,cls_loss,offset_loss,push,pull,angleloss=get_rpn_corner_loss(model, ori_heatmap_lt,rpn_heatmap_lt_label, ori_heatmap_rb,rpn_heatmap_rb_label,offset_lt,offset_rb,ev,inputs,rpn_lt_label,rpn_rb_label,select_pts_lt,select_pts_rb,id2box,pri_angle,angle_label,tb_dict)
            loss += rpn_loss
            #disp_dict['rpn_loss'] = rpn_loss.item()
            disp_dict['cls_loss'] = cls_loss.item()
            disp_dict['offset_loss'] = offset_loss.item()
            disp_dict['chose']=(push+pull.item())*0.1
            disp_dict['angleloss']=angleloss.item()
        if cfg.RCNN.ENABLED:
    ...
    def get_rpn_corner_loss(model, heatmap_lt,rpn_heatmap_lt_label, heatmap_rb,rpn_heatmap_rb_label,offset_lt,offset_rb,ev,pts,rpn_lt_label,rpn_rb_label,id_lt,id_rb,id2box,pri_angle,angle_label,tb_dict):
        loss=0
        cls_loss=0
        offset_loss=0
        #print(heatmap_lt.shape,rpn_heatmap_lt_label.shape)
        cls_loss+=loss_utils.c_focal_loss(heatmap_lt.view(-1),rpn_heatmap_lt_label.view(-1))
        cls_loss+=loss_utils.c_focal_loss(heatmap_rb.view(-1),rpn_heatmap_rb_label.view(-1))
        #b,_=heatmap_lt.shape
        '''
        for i in range(b):
            if i==0:
                pts_lt_label=torch.index_select(rpn_lt_label[i],0,id_lt[i])
                pts_rb_label=torch.index_select(rpn_rb_label[i],0,id_rb[i])
                pts_lt = torch.index_select(pts[i], 0, id_lt[i])
                pts_rb = torch.index_select(pts[i], 0, id_rb[i])
            else:
                pts_lt_label=torch.cat((pts_lt_label,torch.index_select(rpn_lt_label[i],0,id_lt[i])),0)
                pts_rb_label= torch.cat((pts_rb_label, torch.index_select(rpn_rb_label[i], 0, id_rb[i])),
                                       0)
                pts_lt= torch.cat((pts_lt, torch.index_select(pts[i], 0, id_lt[i])), 0)
                pts_rb = torch.cat((pts_rb, torch.index_select(pts[i], 0, id_lt[i])), 0)
        '''
        pts_lt=pts[id_lt]
        pts_rb=pts[id_rb]
        #print(id_lt)
        ...
        for B in id2box:
            cc_box=0
            for box in B:
                idx=torch.from_numpy(B[box]).cuda(non_blocking=True).byte()
                idx_lt=idx.mul(id_lt)
                idx_rb=idx.mul(id_rb)
                ev_lt=ev[idx_lt]
                ev_rb=ev[idx_rb]
                #print(ev_lt.shape,ev_rb.shape)
                nlt,_=ev_lt.shape
                nrb,_=ev_rb.shape
                if nlt==0 or nrb==0:
                    continue
                ...
                ev_lt=torch.sum(ev_lt,0)/nlt
                ev_rb=torch.sum(ev_rb,0)/nrb

                ev_mean = (ev_lt + ev_rb) / 2
                if cc_box == 1:
                    ll=ev_mean.unsqueeze(0)
                else:
                    ll=torch.cat((ll,ev_mean.unsqueeze(0)),0)
                pull += torch.pow(ev_lt - ev_mean, 2).sum() + torch.pow(ev_rb - ev_mean, 2).sum()
            try:
                t,_=ll.shape
            except:
                continue
        ...
        pull=pull/(c_box+1e-4)
        offset_loss+=F.smooth_l1_loss(offset_lt.view(-1,3),(rpn_lt_label-pts_lt))
        offset_loss += F.smooth_l1_loss(offset_rb.view(-1, 3), (rpn_rb_label - pts_rb))
        angle_label=angle_label[(id_lt+id_rb)]
        pri_angle=pri_angle[(id_lt+id_rb)]
        #print(angle_label.shape,pri_angle.shape)
        ###angle loss
        angleloss=loss_utils.get_angleloss(pri_angle,angle_label,cfg.RPN.NUM_HEAD_BIN)

        ###
        loss=cls_loss+offset_loss+0.1*(push+pull)+angleloss
        tb_dict.update({'rpn_loss_cls': cls_loss.item(), 'rpn_offset_loss': offset_loss.item(),
                        'rpn_push': push, 'rpn_pull':pull.item()})
        return loss,cls_loss,offset_loss,push,pull,angleloss
    def get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict,points):
        #print(points.size())  # B,N,3
        ...
        #print(points.size(),point_num)
        fg_sum = fg_mask.long().sum().item()
        if fg_sum != 0:
            loss_loc, loss_angle, loss_size, reg_loss_dict = \
                loss_utils.get_reg_loss_rpn(rpn_reg.view(point_num, -1)[fg_mask],
