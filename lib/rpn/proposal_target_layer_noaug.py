...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict,stage=1):
        ...
        if stage == 1:
            self.reg_fg_thresh = cfg.RCNN.REG_FG_THRESH
            self.cls_fg_thresh = cfg.RCNN.CLS_FG_THRESH
            self.bg_thresh_hi = cfg.RCNN.CLS_BG_THRESH

        elif stage == 2:
            self.reg_fg_thresh = cfg.RCNN.REG_FG_THRESH_2ND
            self.cls_fg_thresh = cfg.RCNN.CLS_FG_THRESH_2ND
            self.bg_thresh_hi = cfg.RCNN.CLS_BG_THRESH_2ND
        elif stage == 3:
            self.reg_fg_thresh = cfg.RCNN.REG_FG_THRESH_3RD
            self.cls_fg_thresh = cfg.RCNN.CLS_FG_THRESH_3RD
            self.bg_thresh_hi = cfg.RCNN.CLS_BG_THRESH_3RD
        roi_boxes3d=torch.cat((roi_boxes3d,gt_boxes3d),1)
        batch_rois, batch_gt_of_rois, batch_roi_iou = self.sample_rois_for_rcnn(roi_boxes3d, gt_boxes3d,stage)
        ...
        '''
        real_gt=batch_gt_of_rois.clone()
        ...
        invalid_mask = (batch_roi_iou > self.bg_thresh_hi) & (batch_roi_iou < self.cls_fg_thresh)
        ...
        output_dict = {'sampled_pts': sampled_pts.view(-1, cfg.RCNN.NUM_POINTS, 3),
                       ...
                       'real_gt':real_gt.view(-1,7)}

        return output_dict

    def sample_rois_for_rcnn(self, roi_boxes3d, gt_boxes3d,stage):
        """
        ...
        for idx in range(batch_size):
            ...
            hard_bg_inds = torch.nonzero((max_overlaps < self.bg_thresh_hi) &
            ...
            if fg_num_rois > 0 and bg_num_rois > 0:
                ...
                #### add to constant fg roi

                if fg_rois_per_this_image < fg_rois_per_image:
                    diff = fg_rois_per_image - fg_rois_per_this_image
                    add = torch.from_numpy(np.random.randint(0, fg_num_rois, (diff))).type_as(gt_boxes3d).long()
                    add_inds=fg_inds[add]
                    #print(add.shape,add_inds.shape,fg_inds.shape)
                    fg_inds=torch.cat((fg_inds,add_inds),0)

                # sampling bg
                bg_rois_per_this_image = cfg.RCNN.ROI_PER_IMAGE - fg_rois_per_image
            ...
            if fg_rois_per_this_image > 0:
                ...
                roi_list.append(fg_rois_src)
                roi_iou_list.append(iou3d_src)
