...
def eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        #print(rpn_scores_raw.is_cuda, rpn_reg.is_cuda)
        # proposal layer
...
def eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir, logger):
    ...
    final_output_dir2 = os.path.join(result_dir, 'final_result_2', 'data')
    ...
    for data in dataloader:
        ...
        ret_dict = model(input_data)
        rcnn_cls = ret_dict['rcnn_cls_3rd']
        rcnn_reg = ret_dict['rcnn_reg_3rd']
        pred_boxes3d=ret_dict['pred_boxes3d_3rd'].squeeze(1)
        #pred_boxes3d=input_data['roi_boxes3d']
        ...
        if not args.test:
            ...
            # calculate recall
            #print(input_data['roi_boxes3d'],gt_boxes3d)
            #print(pred_boxes3d,gt_boxes3d)
            gt_num = gt_boxes3d.shape[0]
            if gt_num > 0:
                iou3d = iou3d_utils.boxes_iou3d_gpu(pred_boxes3d, gt_boxes3d)
                gt_max_iou, _ = iou3d.max(dim=0)
                #print(gt_max_iou.item())
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    print('joint')
    final_output_dir = os.path.join(result_dir, 'final_result', 'data')
    final_output_dir2 = os.path.join(result_dir, 'final_result_iounms', 'data')

    os.makedirs(final_output_dir, exist_ok=True)
    os.makedirs(final_output_dir2, exist_ok=True)
    if args.save_result:
        roi_output_dir = os.path.join(result_dir, 'roi_result', 'data')
        refine_output_dir = os.path.join(result_dir, 'refine_result', 'data')
    ...
    if bug==1:
        split_file = os.path.join(dataset.imageset_dir, '..', '..', 'ImageSets', dataset.split + '.txt')
        split_file = os.path.abspath(split_file)
        logger.info('Averate Precision_iouNMS:')
        name_to_class = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2}
        ap_result_str, ap_dict = kitti_evaluate(dataset.label_dir, final_output_dir2, label_split_file=split_file,
                                                current_class=name_to_class[cfg.CLASSES])
        logger.info(ap_result_str)
    ...
    for data in dataloader:
        ...
        rcnn_cls = ret_dict['rcnn_cls_3rd'].view(batch_size, -1, ret_dict['rcnn_cls'].shape[1])
        #rcnn_reg = ret_dict['rcnn_reg'].view(batch_size, -1, ret_dict['rcnn_reg'].shape[1])  # (B, M, C)
        #pred=ret_dict['pred_boxes3d_2nd']
        pred_boxes3d=ret_dict['pred_boxes3d_3rd']
        # bounding box regression
        anchor_size = MEAN_SIZE
        if cfg.RCNN.SIZE_RES_ON_ROI:
            assert False
        ...
        if rcnn_cls.shape[2] == 1:
            ...
            #raw_scores = rcnn_cls.mul(ret_dict['pre_iou'].view(batch_size, -1, ret_dict['rcnn_cls'].shape[1]))
            if iou_nms == True:
                raw_scores2=rcnn_cls.mul(ret_dict['pre_iou3'].view(batch_size, -1, ret_dict['rcnn_cls'].shape[1]))
                #print(ret_dict['pre_iou'])
                #print(raw_scores.shape, raw_scores2.shape)
                norm_scores2=torch.sigmoid(raw_scores2)
            norm_scores = torch.sigmoid(raw_scores)
            pred_classes = (norm_scores > cfg.RCNN.SCORE_THRESH).long()
            #print(norm_scores2.shape,norm_scores.shape)
        else:
            pred_classes = torch.argmax(rcnn_cls, dim=1).view(-1)
            cls_norm_scores = F.softmax(rcnn_cls, dim=1)
            raw_scores = rcnn_cls[:, pred_classes]
        ...
        inds = norm_scores > cfg.RCNN.SCORE_THRESH
        if iou_nms == True:
            inds2=norm_scores2 > cfg.RCNN.SCORE_THRESH
        #print(inds.shape,inds2.shape)
        #print('inds')
        ...
        for k in range(batch_size):
            ...
            #print(pred_boxes3d_selected.shape)
            if iou_nms == True:
                cur_inds2 = inds2[k].view(-1)
                pred_boxes3d_selected2 = pred_boxes3d[k, cur_inds2]
                raw_scores_selected2 = raw_scores2[k, cur_inds2]
                norm_scores_selected = norm_scores2[k, cur_inds2]
                #print( pred_boxes3d_selected2.shape)
                # NMS thresh
                # rotated nms
                boxes_bev_selected2 = kitti_utils.boxes3d_to_bev_torch(pred_boxes3d_selected2)
                keep_idx2 = iou3d_utils.nms_gpu(boxes_bev_selected2, raw_scores_selected2, cfg.RCNN.NMS_THRESH).view(-1)
                #print(keep_idx.shape,keep_idx2.shape)
                pred_boxes3d_selected2 = pred_boxes3d_selected2[keep_idx2]
                scores_selected2 = raw_scores_selected2[keep_idx2]
                pred_boxes3d_selected2, scores_selected2 = pred_boxes3d_selected2.cpu().numpy(), scores_selected2.cpu().numpy()

                cur_sample_id = sample_id[k]
                #print(pred_boxes3d_selected.shape)
                calib = dataset.get_calib(cur_sample_id)
                final_total += pred_boxes3d_selected2.shape[0]
                image_shape = dataset.get_image_shape(cur_sample_id)
                save_kitti_format(cur_sample_id, calib, pred_boxes3d_selected2,final_output_dir2, scores_selected2,
                                  image_shape)
...
def eval_single_ckpt(root_result_dir):
    ...
    model = cat(num_classes=test_loader.dataset.num_class, use_xyz=True, mode='TEST')
    model.cuda()
...
def create_dataloader(logger):
    mode = 'TEST' if args.test else 'EVAL'
    DATA_PATH = os.path.join('/home/hjx/Desktop/PointRCNN-old/', 'data')
